...
# Branch lane colors: black, red, green, blue.
LANE_PALETTE = [
    (0, 0, 0),       # #000000
    (255, 0, 0),     # #FF0000
    (0, 255, 0),     # #00FF00
...
def render_lineage_dag(
    ...
    ``order`` lists the ids oldest-first: the top of the image is the oldest
    commit and history flows downward.
    ...
    # Assign a column (lane) per lane key, in first-appearance order.
    col_of_lane: dict[str, int] = {}
    for cid in order:
        k = nodes[cid]["lane"]
        if k not in col_of_lane:
            col_of_lane[k] = len(col_of_lane)
    ncols = max(1, len(col_of_lane))

    # Row = longest path from a root, so siblings (e.g. both sides of a
    # branch) share a row and the merge below forms a clear diamond.
    row_of: dict[str, int] = {}
    for cid in order:
        parents = [p for p in nodes[cid]["parents"] if p in row_of or p in nodes]
        known = [row_of[p] for p in parents if p in row_of]
        row_of[cid] = (1 + max(known)) if known else 0
    nrows = 1 + max(row_of.values())
    ...
    width = int(left * 2 + (ncols - 1) * lane_w + iw)
    height = int(top + (nrows - 1) * row_h + ih + 48)

    img = Image.new("RGB", (width, height), BG_COLOR)
    draw = ImageDraw.Draw(img)

    def lane_color(cid: str) -> tuple[int, int, int]:
        return LANE_PALETTE[col_of_lane[nodes[cid]["lane"]] % len(LANE_PALETTE)]

    def image_box(cid: str) -> tuple[int, int, int, int]:
        col = col_of_lane[nodes[cid]["lane"]]
        x = left + col * lane_w
        y = top + row_of[cid] * row_h
        return x, y, x + iw, y + ih

    # Lines: each parent connects down to its child. First-parent edges use
    # the child's lane color; extra (merge-in) parents keep their own lane
    # color so the side branch visibly rejoins.
    for cid in order:
        parents = [p for p in nodes[cid]["parents"] if p in row_of]
        cbx0, cby0, cbx1, _ = image_box(cid)
        end = ((cbx0 + cbx1) // 2, cby0)
        for i, p in enumerate(parents):
            pbx0, _, pbx1, pby1 = image_box(p)
...
def render_lineage(repo: Path, commits: dict[str, Commit], order: list[str], refs: dict, out_path: Path) -> None:
    ...
    render_lineage_dag(nodes, order, out_path)
